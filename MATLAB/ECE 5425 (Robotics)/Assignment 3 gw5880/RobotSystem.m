    ...
    methods
        ...
        function moveToPosition(obj, variables)
            ...
            if(~isempty(obj.device))
                ...
                obj.ser
                vo1.writePosition(servoValues(1));
        ...
        function obj = lowerOrRaiseToWrite(obj)
            ...
            if(currentZ-.1/1000 <= obj.floorZactual) 
                % Marker is lowered, so raise pen by 10 mm 
        ...
        function drawPath(obj, path)
            % Function that draws a path generated by a path planning
            % algorithm

            % Prepare robot by moving EE to the path's starting position
            currentXY = obj.model.fkine(obj.model.getpos()).t(1:2);
            XYchange = path(1,:)/1000 - currentXY';

            obj.moveEE([XYchange, 0], 1);

            % Seperating the X and Y values
            smallChangeInX = path(:,1)/1000;
            smallChangeInY = path(:,2)/1000;

            % Lower marker against the clipboard
            obj.lowerOrRaiseToWrite();

            for i = 1:numrows(path)
                % For loop that simply moves the EE to the small changes in
                % the X and Y directions in each step that will
                % collectively result in a path being drawn
                   obj.moveTo([smallChangeInX(i), smallChangeInY(i)])
            end
